...
function addStepRow(modal, prefill = null) {
  ...
  function setState(modal, recipe) {
    ...
    // Tags
    let tagsWrap = $id("rmTags", modal);
    // Ensure input + button exist and sit before the chips area
    (function ensureTagControls() {
      if (!tagsWrap) return;
      let input = $id("rmTagInput", modal);
      let addBtn = $id("rmAddTag", modal);
      if (!input || !addBtn) {
        ...
        addBtn = addBtn || document.createElement("button");
        addBtn.type = "button";
        addBtn.id = "rmAddTag";
        ...
        try {
          tagsWrap.parentNode.insertBefore(controls, tagsWrap);
        } catch {
          // Fallback: prepend to modal if rmTags parent is missing
          modal.insertBefore(controls, modal.firstChild);
        }
      }
    })();
    ...
    if (tagInput && !tagInput.__wiredEnter) {
      tagInput.__wiredEnter = true;
...
function wireOnce(modal) {
  ...
  if (saveBtn) saveBtn.addEventListener("click", async (e) => {
    ...
    // Build payload and target ref first so we have an id for optimistic insert
    const existingId = str(modal.__editingId || "");
    const draft = buildState(modal);
    const ref = existingId ? recipesCol().doc(existingId) : recipesCol().doc();
    const id = existingId || ref.id;
    modal.__editingId = id; // persist for any follow-up handlers

    const payload = { ...draft, updatedAt: serverTS() };
    if (!existingId) payload.createdAt = serverTS();

    // ---- Optimistic update: stage into local cache and repaint ----
    const prevCache = Array.isArray(window.lastSnapshotRecipes) ? window.lastSnapshotRecipes.slice() : [];
    let nextCache = prevCache.slice();
    const idx = nextCache.findIndex(r => String(r?.id || "") === id);
    const optimistic = { id, ...draft, _syncing: true };

    if (idx >= 0) nextCache[idx] = { ...nextCache[idx], ...optimistic };
    else nextCache.unshift(optimistic);

    try {
      window.lastSnapshotRecipes = nextCache;
      try { localStorage.setItem("cache_recipes", JSON.stringify(nextCache)); } catch {}
      try {
        if (typeof window.scheduleRenderRecipes === "function") window.scheduleRenderRecipes(nextCache);
        else if (typeof window.setRecipesAndRepaint === "function") window.setRecipesAndRepaint(nextCache);
      } catch {}
    } catch {}

    // Close immediately; background write proceeds
    closeUI(modal);
    ...
    try {
      ...
      const j = cached.findIndex(r => String(r?.id || "") === id);
      if (j >= 0) {
        const cleaned = { ...cached[j] };
        try { delete cleaned._syncing; } catch {}
        // Keep latest draft values; server snapshot will reconcile soon
        cached[j] = cleaned;
      }
      window.lastSnapshotRecipes = cached;
      try { localStorage.setItem("cache_recipes", JSON.stringify(cached)); } catch {}
      try {
        if (typeof window.scheduleRenderRecipes === "function") window.scheduleRenderRecipes(cached);
        else if (typeof window.setRecipesAndRepaint === "function") window.setRecipesAndRepaint(cached);
      } catch {}
      ...
      // Roll back cache
      try {
        window.lastSnapshotRecipes = prevCache;
        try { localStorage.setItem("cache_recipes", JSON.stringify(prevCache)); } catch {}
        try {
          if (typeof window.scheduleRenderRecipes === "function") window.scheduleRenderRecipes(prevCache);
          else if (typeof window.setRecipesAndRepaint === "function") window.setRecipesAndRepaint(prevCache);
        } catch {}
      } catch {}

      try { window.showToast?.("Save failed. Changes reverted."); } catch {}
    }
  }, { capture: true });
  ...
  if (delBtn) delBtn.addEventListener("click", async (e) => {
    ...
    const prevCache = Array.isArray(window.lastSnapshotRecipes) ? window.lastSnapshotRecipes.slice() : [];
    const nextCache = prevCache.filter(r => String(r?.id || "") !== id);
    ...
    // Close immediately; background delete proceeds
    closeUI(modal);
    ...
    try {
      ...
      // Leave cache as-is; server snapshot will reconcile
