...
if (rmDelete){
  rmDelete.onclick = async (ev) => {
    ...
    // Hard delete with optimistic remove + revert on failure
    ...
    try{
      if (Array.isArray(lastSnapshotRecipes)){
        removedIndex = lastSnapshotRecipes.findIndex(r => r && r.id === editingRecipeId);
        if (removedIndex >= 0) removedRecipe = lastSnapshotRecipes[removedIndex];
      }
    }catch{}
    ...
    closeRecipeModal();
    try{
      if (Array.isArray(lastSnapshotRecipes) && removedIndex >= 0){
        lastSnapshotRecipes = lastSnapshotRecipes.filter(r => r && r.id !== editingRecipeId);
        if (typeof scheduleRenderRecipes === 'function') scheduleRenderRecipes(lastSnapshotRecipes);
      }
      showToast && showToast('Deleting…');
    ...
    try {
      ...
      // Revert UI
      try{
        if (Array.isArray(lastSnapshotRecipes) && removedRecipe){
          if (removedIndex < 0 || removedIndex > lastSnapshotRecipes.length) {
            lastSnapshotRecipes = [...lastSnapshotRecipes, removedRecipe];
          } else {
            lastSnapshotRecipes = [
              ...lastSnapshotRecipes.slice(0, removedIndex),
              removedRecipe,
              ...lastSnapshotRecipes.slice(removedIndex)
            ];
          }
          if (typeof scheduleRenderRecipes === 'function') scheduleRenderRecipes(lastSnapshotRecipes);
        }
