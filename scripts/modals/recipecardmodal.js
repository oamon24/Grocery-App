...
function wireOnce(modal) {
  ...
  if (saveBtn) saveBtn.addEventListener("click", async (e) => {
    ...
    // Build data and target ref
    let id = str(modal.__editingId || "");
    const data = buildState(modal);
    const creating = !id;
    const ref = creating ? recipesCol().doc() : recipesCol().doc(id);
    if (creating) id = ref.id;

    // Prepare optimistic object
    const nowMs = Date.now();
    const optimistic = {
      id,
      ...data,
      updatedAt: nowMs,
      ...(creating ? { createdAt: nowMs } : {}),
      _syncing: true
    };
    ...
    // Apply optimistic update to cache and UI
    try {
      const next = prev.slice();
      const i = next.findIndex(r => String(r?.id || "") === id);
      if (i >= 0) next[i] = { ...next[i], ...optimistic };
    ...
    try {
      const payload = {
        ...data,
        updatedAt: serverTS(),
        ...(creating ? { createdAt: serverTS(), id } : {})
      };
      ...
      if (j >= 0) {
        ...
        try {
          if (typeof window.setRecipesAndRepaint === "function") window.setRecipesAndRepaint(cur);
          else if (typeof window.scheduleRenderRecipes === "function") window.scheduleRenderRecipes(cur);
          else window.__recipesPending = cur;
        } catch {}
      }
