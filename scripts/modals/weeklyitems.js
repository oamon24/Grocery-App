...
window.__wiInit = true;
  ...
  function renderWiRow(it){
    ...
    // Unified handler: require a real items doc id, mirror to weekly, update UI
      ...
      try {
        ...
        try {
          ...
          if (window.db && hh) {
            ...
            if (snap && snap.exists) {
              const d = snap.data() || {};
              prevPath = d.photoPath || null;
            }
          }
        } catch (e) { console.warn("[weeklyItems] prevPath lookup failed", e); }

        const { url, path } = await photo.uploadItemPhoto(file, itemId, prevPath || null);

        // Mirror into weekly doc so this modal stays in sync; also persist itemId mapping if absent
        await wiDocRef(it.id).set({
          itemId,
          photoUrl: url,
          photoPath: path,
        ...
        // Swap button → img if needed, or refresh existing img
                  ...
                  onCamera: () => {
                    try {
                      photo.openPhotoPicker({
                        itemId,
                        onFile: async (f) => { await onPicked(f); }
                      });
  ...
  // Sync weekly rows when an item's photo changes elsewhere
  document.addEventListener("item:photo-updated", async (ev) => {
    try {
      ...
      for (const w of all) {
        // If this weekly doc was not linked, persist the linkage first
        if (!String(w.itemId || "").trim()) {
          w.itemId = itemId;
          try {
            await wiDocRef(w.id).set(
              { itemId, updatedAt: firebase.firestore.FieldValue.serverTimestamp() },
              { merge: true }
            );
          } catch (_) {}
        ...
        // Update local model with latest photo
        w.photoUrl = url;
        w.photoPath = path;

        // Mirror into weekly doc so modal stays in sync
        try {
          await wiDocRef(w.id).set(
        ...
        // Patch the visible row if rendered
        if (listEl) {
