  ...
  async function onAdd(){
    ...
    try {
      ...
      if (existing){
        ...
        // Optimistic update with captured previous for revert
        let idx = -1;
        let prev = null;
        try {
          idx = window.lastSnapshotItems.findIndex(x => x && x.id === existing.id);
          if (idx >= 0){
            prev = { ...window.lastSnapshotItems[idx] };
        ...
        try {
          ...
          try {
            if (idx >= 0 && prev && Array.isArray(window.lastSnapshotItems)) {
              window.lastSnapshotItems[idx] = prev;
              if (typeof window.scheduleRenderItems === 'function') window.scheduleRenderItems(window.lastSnapshotItems);
            }
          } catch(_){}
          try { window.showToast && window.showToast('Merge failed'); } catch(_){}
          throw e;
        }
      ...
      // Background write with server timestamps
      try {
        await ref.set({
          name: doc.name,
          nameKey: doc.nameKey,
        ...
        try {
          if (inserted && Array.isArray(window.lastSnapshotItems)) {
            window.lastSnapshotItems = window.lastSnapshotItems.filter(it => it && it.id !== doc.id);
            if (typeof window.scheduleRenderItems === 'function') window.scheduleRenderItems(window.lastSnapshotItems);
          }
        } catch(_){}
        throw e;
