...
(function(){
  ...
  function ensureModal(){
    ...
    if (!$("atlStyles")) {
      ...
      #atlList{display:grid;grid-template-columns:1fr auto;gap:6px;max-height:45vh;overflow:auto;border-top:1px solid var(--border);padding-top:8px}
    ...
    modal.innerHTML = `
      <div class="sheet" role="dialog" aria-modal="true" aria-labelledby="atlTitle">
        ...
        <div class="row" style="display:flex;gap:8px;align-items:center;margin-bottom:8px;">
          ...
          <input id="atlServings" type="number" min="1" step="1" value="1" style="width:92px">
  ...
  function renderRows(){
    ...
    for (const r of current.rows){
      ...
      list.appendChild(name); list.appendChild(qty);
    }
  }
  ...
  async function onConfirm(){
    try {
      ...
      for (const r of current.rows){
        ...
        if (found) {
          const prevQty = String(found.qty || "").trim();
          const nextQty = mergeQtyUnitAware(prevQty, r.qty);
      ...
      if (touchedExisting && typeof window.scheduleRenderItems === "function") {
        try { window.scheduleRenderItems(window.lastSnapshotItems || existing); } catch {}
      }
