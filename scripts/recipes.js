    ...
    window.setRecipesAndRepaint = function setRecipesAndRepaint(arr) {
      ...
      try {
        ...
        if (el) {
          ...
          el.textContent = n === 1 ? "1 recipe" : `${n} recipes`;
        }
      } catch {}
...
function getCommonTags() {
  ...
  // sort by frequency desc, then alpha; cap list size
...
function renderTagsUI(preset = []) {
  if (!tagsRow) return;
  const selected = new Set((Array.isArray(preset) ? preset : []).map(s => String(s)));
  ...
  rmDelete.addEventListener("click", async () => {
    try {
      ...
      try {
        ...
        localStorage.setItem("cache_recipes", JSON.stringify(next));
